...
def _put_rotated_text_right(
    ...
    *,
    ...
    cv2.putText(
        canvas, text, (pad, th + pad), font, font_scale, color, thickness, cv2.LINE_AA
    )
    ...
    *,
    ...
    while lat <= lat_max + 1e-12:
        ...
        _put_rotated_text_right(
            img, label, (right + 8, p2[1] - 20), font=font, font_scale=0.45, thickness=1
        )
    ...
    for plat in overlay.platform_points:
        ...
        cv2.putText(
            img, plat.name, (px + 10, py + 5), font, 0.45, (0, 0, 0), 1, cv2.LINE_AA
        )
